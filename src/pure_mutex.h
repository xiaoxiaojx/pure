#ifndef SRC_PURE_MUTEX_H_
#define SRC_PURE_MUTEX_H_
...
class ExclusiveAccess {
 public:
  ...
  // explicit 指定符可以与常量表达式一同使用. 函数若且唯若该常量表达式求值为
  // true 才为显式. (C++20起)
...
struct LibuvMutexTraits {
  ...
  static inline void mutex_rdlock(MutexT* mutex) { uv_mutex_lock(mutex); }

  static inline void mutex_rdunlock(MutexT* mutex) { uv_mutex_unlock(mutex); }
};

struct LibuvRwlockTraits {
  using MutexT = uv_rwlock_t;

  static inline int mutex_init(MutexT* mutex) { return uv_rwlock_init(mutex); }

  static inline void mutex_destroy(MutexT* mutex) { uv_rwlock_destroy(mutex); }

  static inline void mutex_lock(MutexT* mutex) { uv_rwlock_wrlock(mutex); }

  static inline void mutex_unlock(MutexT* mutex) { uv_rwlock_wrunlock(mutex); }

  static inline void mutex_rdlock(MutexT* mutex) { uv_rwlock_rdlock(mutex); }
...
MutexBase<Traits>::MutexBase() {
  assert(0 == Traits::mutex_init(&mutex_));
}
