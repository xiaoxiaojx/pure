...
template <typename T>
void DateTimeConfigurationChangeNotification(Isolate* isolate,
                                             const T& key,
...
MaybeLocal<String> RealEnvStore::Get(Isolate* isolate,
  ...
  Utf8Value key(isolate, property);
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (str.IsEmpty()) {
      isolate->ThrowException(
          ERR_STRING_TOO_LONG(isolate, "ERR_STRING_TOO_LONG"));
      return Local<Array>();
    }
...
Local<Array> MapKVStore::Enumerate(Isolate* isolate) const {
  ...
  for (const auto& pair : map_) {
    values.emplace_back(String::NewFromUtf8(isolate,
                                            pair.first.data(),
...
Maybe<bool> KVStore::AssignFromObject(Local<Context> context,
  ...
  for (uint32_t i = 0; i < keys_length; i++) {
    Local<Value> key;
    if (!keys->Get(context, i).ToLocal(&key)) return Nothing<bool>();
...
static void EnvSetter(Local<Name> property,
  ...
  if (env->options()->pending_deprecation && !value->IsString() &&
    ...
    //         " "process.env property is deprecated. Please make sure to
    //         convert " "the " "value to a string before setting process.env
    //         with it.", "DEP0104") .IsNothing())
    return;
  }
...
static void EnvDefiner(Local<Name> property,
  ...
  Environment* env = Environment::GetCurrent(info);
  if (desc.has_value()) {
    if (!desc.has_writable() || !desc.has_enumerable() ||
